...
def BNI_LI_with_teleport(G:nx.Graph, coverage:float=0.01, rw_reps:float = 0.05, doLocalProbing=False, verbose:bool=False, maxiter:int=1000):
    """Finds nodes to immunize in a given network G via Bridge-Neighbourhood Immunization with Local Information (BNI-LI) algorithm.
    Uses teleportation if it hits a dead end in the self-avoiding random walk.    
    ...
    for trial in range(R):   
        ...
        if not rw_options: # if we hit a dead end: warn & teleport
            warnings.warn("Houston, we have a problem: no options to continue the walk. Teleporting to different node and rebuilding friendship circle.")
            if verbose:
                print(f"Random walk cannot continue from node {node_curr}. Teleporting to random node.")
            initSuccessful, node_curr, rw_options, F, RW = run_init(RW) # teleport to random node, redo the initialisation steps of friendship circle, but keep the og RW steps
            if verbose:
